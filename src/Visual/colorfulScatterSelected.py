...
def doPaintNorm():
    targetList = ["inakamai", "inaurora", "inphys", "incampus", "incampusNew",
    ...
    for target in targetList:
        ...
        foldernametotal = "../../exchange/total/"
        filenametotal = "%sTotalOutByteClusterCollTen_trans.log" % target
        # ilenametotal = "%sTotalInByteClusterCollTen_trans.log" % target
        ...
        plt.scatter(logScatterMean, logScatterStd, color=getModuleColor(target),
                    marker=getMarkerStyle(target), linewidths=2, label=target)
    plt.title("Outbound Volume by each IPCluster, Top 1000 for each mdoule")
    plt.xlim((-1, 7))
...
def getModuleColor(moduleName):
    moduleColorDict = {"inakamai": "y", "inaurora": "purple", "incampus": "firebrick", "incampusNew": "maroon",
                       "incpsc": "blue", "inothers": "slategray", "inphys": "magenta", "inunknown205": "silver",
                       "outakamai": "y", "outcampus1": "firebrick", "outcampus2": "maroon",
                       "outcpsc": "blue", "outothers": "slategray", "outwebpax": "lightgray" }
    # print("Cur %s Color is %s" % (moduleName, moduleColorDict[moduleName]))
    return moduleColorDict[moduleName]


def getMarkerStyle(moduleName):
    if moduleName.startswith("in"):
        return "+"
    elif moduleName.startswith("out"):
        return "v"
    else:
        print("Warning!, Invalid moduleName in MarkerStyle")
        return "X"
