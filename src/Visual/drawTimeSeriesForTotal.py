...
class TimeSeriesPlot:
    ...
    def paint(self, y_for_in=None):
        data_key_list = self._get_data_key_list()
        data_dict = self._data_loader()
        y_data_end = []
        for data_key in data_key_list:
            try:
                y_data_end.append(data_dict[data_key])
            except KeyError:
                print("Specified data key %s is not found from data dict" % data_key)
                print("Error data range or Error input src file")

        x_data = list(range(len(y_data_end)))

        # self.ax.plot(x_data, y_data, label=self.module_name, color=color_dict[self.module_name])
        if self.module_name is "in":
            y_data_start = [0] * len(y_data_end)
            self.ax.fill_between(x_data, y_data_start, y_data_end, label="Total Inbound Count", color="red", linewidth=1)
            return y_data_end
        elif self.module_name is "out":
            if y_for_in is not None:
                y_data_start = y_for_in
                y_data_end = [x+y for x, y in zip(y_data_start, y_data_end)]
                self.ax.fill_between(x_data, y_data_start, y_data_end, label="Total Outbound Count", color="blue",
                                     linewidth=1)
                self.decoration()
    ...
    def decoration(self):
        ...
        for x_loc in range(24, max_x_lim, 24):
            self.ax.plot([x_loc, x_loc], [0, max_y_lim], linewidth=0.5, color="black", linestyle="--")

        self.ax.set_ylim(0
                         , max_y_lim)
        self.ax.set_xlim(0, max_x_lim)
        self.ax.set_xticks(list(range(0, max_x_lim+24, 24)))
        self.ax.set_xticklabels(["2018-09-%s" % str(d).zfill(2) for d in range(3, 11)], rotation=15)

        self.ax.set_yticks(list(range(0, max_y_lim+1, 200000)))
        self.ax.set_yticklabels(["0"] + ["$%0.0fx10^{6}$" % base for base in np.arange(2, 28, 2)])

        self.ax.set_ylabel("Connection Count/hour", fontsize=15)
        ...
        while i < len(labels):
            if labels[i] in labels[:i]:
                ...
                i += 1
        plt.legend(reversed(handles), reversed(labels), loc="best")
        self.ax.legend(reversed(handles), reversed(labels), loc="best", fontsize=15)
